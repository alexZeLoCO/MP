...
public class Game {
	...
	public Game(int numHeroes, int numMonsters, int rows, int columns, int totalRounds) {
		...
		characters = new Character[numHeroes + numMonsters+3];

		for (int i=0;i<3;i++) {
			characters[i] = new Guardian ("Guard"+i);
		}
		...
		for (int y = 0; y < numMonsters; y++)
			...
			case (4):
				characters[numHeroes + y] = new DirtyRat("Rat" + y);
				break;
	...
	public Game() {
		...
		for (int i=0;i<5;i++) {
			characters[i] = new Guardian("Guardian"+i);
		}
		for (int i=0;i<3;i++) {
			characters[4+i] = new Barbarian ("Barbarian"+i, "player"+i);
		}
		for (int i=0;i<3;i++) {
			characters[7+i] = new Mummy ("Mummy"+i);
		}
		/*
			if (Dice.roll() % 2 == 0)// if even create a mummy
				characters[numHeroes + y] = new Mummy("Mummy" + y);
